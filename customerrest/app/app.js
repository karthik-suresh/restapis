...
app.factory("services", ['$http', function($http) {
  var serviceBase = 'services/'
    ...
    obj.deleteAllCustomers = function (){
      alert('Called');
      return $http.delete(serviceBase + 'deleteAllCustomers').then(function (status) {
          return status.data;
      });
  };
...
app.controller('editCtrl', function ($scope, $rootScope, $location, $routeParams, services, customer) {
    ...
    $scope.buttonText = (customerID > 0) ? 'Update Customer' : 'Add New Customer';
      ...
      $scope.deleteCustomer = function(customer) {
        ...
        $location.path('/');
        if(confirm("Are you sure to delete all customers?")==true)
